...
<div class="glass p-8 rounded-3xl max-w-4xl mx-auto">
    <form action="{{ route('admin.case-studies.update', $caseStudy) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="title" id="title" required value="{{ old('title', $caseStudy->title) }}" placeholder="e.g. Doctor Appointment App Development">
            </div>

            <div>
                <label for="client" class="font-semibold text-slate-700">Client Name</label>
                <input type="text" name="client" id="client" value="{{ old('client', $caseStudy->client) }}" placeholder="e.g. Vrinsoft Medical Care">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="industry_id" id="industry_id" class="w-full">
                    ...
                    @foreach($industries as $industry)
                        <option value="{{ $industry->id }}" {{ old('industry_id', $caseStudy->industry_id) == $industry->id ? 'selected' : '' }}>
                            {{ $industry->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="url" name="link" id="link" value="{{ old('link', $caseStudy->link) }}" placeholder="https://example.com">
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100 max-h-48 overflow-y-auto">
                @php $selectedTechs = old('technologies', $caseStudy->technologies->pluck('id')->toArray()); @endphp
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3" placeholder="Brief card summary description...">{{ old('description', $caseStudy->description) }}</textarea>
        </div>

        @php $cd = $caseStudy->content_data ?? []; @endphp
            ...
            <div class="bg-slate-50 p-6 rounded-2xl mb-6 border border-slate-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="banner_title" id="banner_title" value="{{ old('banner_title', $cd['banner']['title'] ?? '') }}" placeholder="e.g. Revolutionizing Medical Consultations">
                    </div>
                    <div>
                        <label for="banner_subtitle" class="text-xs">Banner Subtitle</label>
                        <input type="text" name="banner_subtitle" id="banner_subtitle" value="{{ old('banner_subtitle', $cd['banner']['subtitle'] ?? '') }}" placeholder="e.g. Scalable telehealth app connecting users and clinic networks.">
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label for="banner_badge" class="text-xs">Badge Tag</label>
                        <input type="text" name="banner_badge" id="banner_badge" value="{{ old('banner_badge', $cd['banner']['badge'] ?? 'CASE STUDY') }}" placeholder="e.g. MOBILE CASE STUDY">
                    </div>
                    <div>
                        <label for="banner_video_url" class="text-xs">Featured Video URL / Explainer URL</label>
                        <input type="text" name="banner_video_url" id="banner_video_url" value="{{ old('banner_video_url', $cd['banner']['video_url'] ?? '') }}" placeholder="e.g. https://www.youtube.com/watch?v=...">
                    </div>
                </div>
                <div>
...
Automated Prescription Management">@if(isset($cd['highlights'])){{ implode("\n", $cd['highlights']) }}@endif</textarea>
                </div>
            </div>
            ...
            <div class="bg-slate-50 p-6 rounded-2xl mb-6 border border-slate-100">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="overview_title" id="overview_title" value="{{ old('overview_title', $cd['overview']['title'] ?? 'Project Overview') }}" placeholder="Project Overview">
                </div>
                <div>
                    <label for="overview_description" class="text-xs">Overview Detailed Content</label>
                    <textarea name="overview_description" id="overview_description" rows="4" placeholder="Detailed text explaining the background, context, and goals of this case study...">{{ old('overview_description', $cd['overview']['description'] ?? '') }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-slate-50 p-6 rounded-2xl mb-6 border border-slate-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="challenge_title" id="challenge_title" value="{{ old('challenge_title', $cd['challenge']['title'] ?? 'The Challenge') }}" placeholder="The Challenge">
                    </div>
                    <div>
                        ...
                        @if(isset($cd['challenge_image']))
                            <div class="mt-2 text-xs text-slate-500 flex items-center gap-2">
                                <span class="bg-slate-200 px-2 py-0.5 rounded">Current:</span>
                                <a href="{{ asset('storage/' . $cd['challenge_image']) }}" target="_blank" class="text-blue-600 underline font-semibold">{{ basename($cd['challenge_image']) }}</a>
                            </div>
                        @endif
                    </div>
                </div>
                <div>
                    ...
                    <textarea name="challenge_description" id="challenge_description" rows="4" placeholder="Detail the business problems, technical obstacles, or user needs you had to solve...">{{ old('challenge_description', $cd['challenge']['description'] ?? '') }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-slate-50 p-6 rounded-2xl mb-6 border border-slate-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="solution_title" id="solution_title" value="{{ old('solution_title', $cd['solution']['title'] ?? 'The Solution') }}" placeholder="The Solution">
                    </div>
                    <div>
                        ...
                        @if(isset($cd['solution_image']))
                            <div class="mt-2 text-xs text-slate-500 flex items-center gap-2">
                                <span class="bg-slate-200 px-2 py-0.5 rounded">Current:</span>
                                <a href="{{ asset('storage/' . $cd['solution_image']) }}" target="_blank" class="text-blue-600 underline font-semibold">{{ basename($cd['solution_image']) }}</a>
                            </div>
                        @endif
                    </div>
                </div>
                <div>
                    ...
                    <textarea name="solution_description" id="solution_description" rows="4" placeholder="Detail how Devent built the custom solution, architectures, and approaches used...">{{ old('solution_description', $cd['solution']['description'] ?? '') }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-slate-50 p-6 rounded-2xl mb-6 border border-slate-100">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="features_title" id="features_title" value="{{ old('features_title', $cd['features_title'] ?? 'Key Features & Capabilities') }}" placeholder="Key Features">
                </div>
                <div>
                    <label class="text-xs font-semibold text-slate-600 mb-2 block">Features List</label>
                    <div id="features-container" class="space-y-2">
                        @php $features = old('features', $cd['features'] ?? []); @endphp
            ...
            <div class="bg-slate-50 p-6 rounded-2xl mb-6 border border-slate-100">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="approach_title" id="approach_title" value="{{ old('approach_title', $cd['approach']['title'] ?? 'Our Methodology & Approach') }}" placeholder="Strategic Approach">
                </div>
                <div class="mb-4">
                    <label for="approach_description" class="text-xs">Approach Description (Paragraph 1)</label>
                    <textarea name="approach_description" id="approach_description" rows="3" placeholder="Explain the project delivery philosophy, agile sprints, or UX mapping...">{{ old('approach_description', $cd['approach']['description'] ?? '') }}</textarea>
                </div>
                <div>
                    <label for="approach_description2" class="text-xs">Approach Description (Paragraph 2 - Optional)</label>
                    <textarea name="approach_description2" id="approach_description2" rows="3" placeholder="Provide extra technical architecture details or collaboration models...">{{ old('approach_description2', $cd['approach']['description2'] ?? '') }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-slate-50 p-6 rounded-2xl mb-6 border border-slate-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="process_title" id="process_title" value="{{ old('process_title', $cd['process_title'] ?? 'Step-by-Step Delivery') }}" placeholder="Timeline Title">
                    </div>
                    <div>
                        <label for="process_subtitle" class="text-xs">Section Subtitle</label>
                        <input type="text" name="process_subtitle" id="process_subtitle" value="{{ old('process_subtitle', $cd['process_subtitle'] ?? 'From Discovery to Final Launch') }}" placeholder="Timeline Subtitle">
                    </div>
                </div>
                <div class="mb-4">
                    ...
                    @if(isset($cd['process_image']))
                        <div class="mt-2 text-xs text-slate-500 flex items-center gap-2">
                            <span class="bg-slate-200 px-2 py-0.5 rounded">Current:</span>
                            <a href="{{ asset('storage/' . $cd['process_image']) }}" target="_blank" class="text-blue-600 underline font-semibold">{{ basename($cd['process_image']) }}</a>
                        </div>
                    @endif
                </div>
                <div>
                    ...
                    <div id="process-container" class="space-y-2">
                        @php $process = old('process', $cd['process'] ?? []); @endphp
            ...
            <div class="bg-slate-50 p-6 rounded-2xl mb-6 border border-slate-100">
                ...
                <div>
                    <label class="text-xs font-semibold text-slate-600 mb-2 block">Metrics (Display key stats achieved)</label>
                    <div id="achievements-container" class="space-y-2">
                        @php $achievements = old('achievements', $cd['achievements'] ?? []); @endphp
            ...
            <div class="bg-slate-50 p-6 rounded-2xl mb-6 border border-slate-100">
                ...
                <div>
                    ...
                    <div id="testimonials-container" class="space-y-2">
                        @php $testimonials = old('testimonials', $cd['testimonials'] ?? []); @endphp
            ...
            <div class="bg-slate-50 p-6 rounded-2xl mb-6 border border-slate-100">
                ...
                <div>
                    <div id="faqs-container" class="space-y-2">
                        @php $faqs = old('faqs', $cd['faqs'] ?? []); @endphp
                        @foreach($faqs as $i => $item)
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2 p-2 bg-white rounded-lg border border-slate-100 row-item relative">
                                ...
                                <input type="text" name="faqs[{{ $i }}][description]" value="{{ $item['description'] }}" class="text-sm py-1 w-full" placeholder="e.g. It relies on HL7 data models.">
                                <button type="button" onclick="this.closest('.row-item').remove()" class="absolute -right-2 -top-2 bg-red-100 text-red-600 rounded-full w-5 h-5 flex items-center justify-center text-xs hover:bg-red-200">×</button>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="bg-slate-50 p-6 rounded-2xl mb-6 border border-slate-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="cta_title" id="cta_title" value="{{ old('cta_title', $cd['cta']['title'] ?? '') }}" placeholder="e.g. Need a similar application?">
                    </div>
                    <div>
                        <label for="cta_subtitle" class="text-xs">CTA 1 Subtitle</label>
                        <input type="text" name="cta_subtitle" id="cta_subtitle" value="{{ old('cta_subtitle', $cd['cta']['subtitle'] ?? '') }}" placeholder="e.g. Consult with our telemedicine product managers.">
                    </div>
                    <div>
                        <label for="cta_button" class="text-xs">CTA 1 Button Text</label>
                        <input type="text" name="cta_button" id="cta_button" value="{{ old('cta_button', $cd['cta']['button'] ?? 'Connect With Us Today') }}" placeholder="e.g. Book Dynamic Consultation">
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label for="cta2_title" class="text-xs">CTA 2 Title</label>
                        <input type="text" name="cta2_title" id="cta2_title" value="{{ old('cta2_title', $cd['cta2']['title'] ?? '') }}" placeholder="e.g. Scale your Healthcare Practice.">
                    </div>
                    <div>
                        <label for="cta2_subtitle" class="text-xs">CTA 2 Subtitle</label>
                        <input type="text" name="cta2_subtitle" id="cta2_subtitle" value="{{ old('cta2_subtitle', $cd['cta2']['subtitle'] ?? '') }}" placeholder="e.g. Let us build a reliable, custom compliance-ready infrastructure.">
                    </div>
                    <div>
                        <label for="cta2_button" class="text-xs">CTA 2 Button Text</label>
                        <input type="text" name="cta2_button" id="cta2_button" value="{{ old('cta2_button', $cd['cta2']['button'] ?? 'Initiate Healthcare R&D') }}" placeholder="e.g. Initiate Healthcare R&D">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-slate-50 p-6 rounded-2xl mb-6 border border-slate-100">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="why_choose_title" id="why_choose_title" value="{{ old('why_choose_title', $cd['why_choose']['title'] ?? 'Why Partner with Devent?') }}" placeholder="Why Partner with Devent?">
                </div>
                <div class="mb-4">
                    <label for="why_choose_description" class="text-xs">Description Summary</label>
                    <textarea name="why_choose_description" id="why_choose_description" rows="3" placeholder="Summarize our value proposition for this project sector...">{{ old('why_choose_description', $cd['why_choose']['description'] ?? '') }}</textarea>
                </div>
                <div class="mb-4">
                    ...
                    @if(isset($cd['why_choose_image']))
                        <div class="mt-2 text-xs text-slate-500 flex items-center gap-2">
                            <span class="bg-slate-200 px-2 py-0.5 rounded">Current:</span>
                            <a href="{{ asset('storage/' . $cd['why_choose_image']) }}" target="_blank" class="text-blue-600 underline font-semibold">{{ basename($cd['why_choose_image']) }}</a>
                        </div>
                    @endif
                </div>
                <div>
...
Multi-Region Scalable Architecture">@if(isset($cd['why_choose_points'])){{ implode("\n", $cd['why_choose_points']) }}@endif</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <div class="mt-1 flex items-center justify-center px-6 pt-5 pb-6 border-2 border-slate-200 border-dashed rounded-2xl bg-slate-50">
                    <div class="space-y-1 text-center">
                        @if($caseStudy->image)
                            <img src="{{ asset('storage/' . $caseStudy->image) }}" class="mx-auto h-24 w-24 object-cover rounded-xl mb-2" alt="Current featured image">
                        @else
            ...
            <div>
                ...
                <input type="number" name="order" id="order" value="{{ old('order', $caseStudy->order) }}">
