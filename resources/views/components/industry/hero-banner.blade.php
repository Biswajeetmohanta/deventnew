...
<section class="bg-slate-950 py-20 lg:py-28 relative overflow-hidden">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
                ...
                <a href="{{ url('/contact') }}" class="inline-flex items-center bg-amber-500 hover:bg-amber-600 text-white font-bold px-8 py-4 rounded-xl transition-all hover:shadow-lg hover:shadow-amber-500/20">
                    Book a Consultation
    ...
    <div class="absolute top-0 right-0 w-96 h-96 bg-amber-500 rounded-full blur-[150px] opacity-5"></div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 mt-12">
        <nav class="flex space-x-2 text-slate-500 text-sm">
            ...
            <span>/</span>
            <a href="{{ url('/industries') }}" class="hover:text-amber-400 transition-colors">Industries</a>
            <span>/</span>
            <span class="text-amber-400">{{ $breadcrumb ?: $title }}</span>
        </nav>
    </div>
</section>
